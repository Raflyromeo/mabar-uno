      ...
      drawCards: (playerId, amount = 1) => {
          set((state) => {
              ...
              const players = state.players.map(p => {
                  if (p.id === playerId) return { ...p, hand: [...p.hand, ...drawn], isUno: false };
                  return p;
              });
      ...
      playCards: (playerId, cardsToPlay, chosenColor = null) => {
          set(state => {
              ...
              cardsToPlay.forEach(card => {
                 ...
                 if (card.value === 'Reverse') { direction *= -1; effectToast = 'REVERSED!'; }
                 if (card.value === 'Skip') { skipNext = true; effectToast = 'SKIPPED!'; }
              });
              ...
              const newState = hand.length === 0
                ? { players, discardPile: [...state.discardPile, ...cardsToPlay], activeColor, winner: playerId }
                : {
                    players,
